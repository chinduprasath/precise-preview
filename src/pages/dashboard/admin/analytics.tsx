...
import { ResponsiveContainer, AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, Legend, BarChart as RechartsBarChart, Bar, PieChart, Pie, Cell } from 'recharts';
...
const AnalyticsPage = () => {
  ...
  const teamStats = {
    ...
    recentActivity: [{
      ...
      user: 'Neha Singh',
      role: 'Operations',
      action: 'Approved 15 new influencers',
      timestamp: '3 hours ago'
    }, {
      id: 3,
      user: 'Raj Malhotra',
      role: 'Support',
      action: 'Resolved 28 support tickets',
      timestamp: '5 hours ago'
    }, {
      id: 4,
      user: 'Priya Verma',
      role: 'Moderator',
      action: 'Flagged 7 inappropriate posts',
      timestamp: '6 hours ago'
    }, {
      id: 5,
      user: 'Vikram Joshi',
      role: 'Finance',
      action: 'Processed 42 payments',
  ...
  const activeUsersData = [{
    period: 'Last 7 Days',
    users: userStats.activeUsers7Days
  }, {
    period: 'Last 30 Days',
    users: userStats.activeUsers30Days
  }];
        ...
        <main className="flex-1 overflow-auto p-6">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
              <StatsCard title="Total Business Users" value={userStats.totalBusinessUsers.toLocaleString()} color="#9b87f5" graphData={userStats.userGrowthData.map(d => d.business)} />
              <StatsCard title="Total Influencers" value={userStats.totalInfluencers.toLocaleString()} color="#7E69AB" graphData={userStats.userGrowthData.map(d => d.influencers)} />
              <StatsCard title="Total Orders" value={orderStats.totalOrders.toLocaleString()} color="#6E59A5" graphData={orderStats.orderTrendData.map(d => d.orders)} />
              <StatsCard title="Service Orders" value={serviceOrderStats.totalOrders.toLocaleString()} color="#1EAEDB" graphData={serviceOrderStats.ordersTrend.map(d => d.total)} />
            </div>
                      ...
                      <div className="h-80">
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={last6MonthsData}>
                            ...
                            <Tooltip />
                            <Legend />
                            <Area type="monotone" dataKey="influencers" stackId="1" stroke="#9b87f5" fill="#9b87f5" />
                            <Area type="monotone" dataKey="business" stackId="1" stroke="#7E69AB" fill="#7E69AB" />
                          </AreaChart>
                        </ResponsiveContainer>
                      </div>
                    </CardContent>
                      ...
                      <div className="h-60">
                        <ResponsiveContainer width="100%" height="100%">
                          <RechartsBarChart data={activeUsersData}>
                            ...
                            <Tooltip formatter={(value, name) => [value, 'Active Users']} labelFormatter={label => `Time Period: ${label}`} />
                            <Bar dataKey="users" fill="#9b87f5" stroke="#7E69AB" strokeWidth={1} radius={[4, 4, 0, 0]} label={{
                            position: 'top',
                            fontSize: 12,
                            fontWeight: 'bold',
                            fill: '#374151'
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <Table>
                        ...
                        <TableBody>
                          {orderStats.topInfluencers.map(influencer => <TableRow key={influencer.id}>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <Table>
                        ...
                        <TableBody>
                          {orderStats.activeBusinesses.map(business => <TableRow key={business.id}>
              ...
              <TabsContent value="services" className="space-y-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    ...
                    {/* Graphic Designers */}
                    {(serviceCategory === 'all' || serviceCategory === 'graphicDesigners') && <div className="mb-8">
                    ...
                    {(serviceCategory === 'all' || serviceCategory === 'socialMedia') && <div className="mb-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                              {Object.entries(serviceStats.socialMedia.platforms).map(([platform, count]) => <div key={platform} className="bg-white rounded-lg border p-3">
                          ...
                          <div className="h-60 bg-white rounded-lg border p-4">
                            <ResponsiveContainer width="100%" height="100%">
                              <PieChart>
                                <Pie data={serviceStats.socialMedia.platformsData} cx="50%" cy="50%" labelLine={false} outerRadius={80} fill="#8884d8" dataKey="value" label={({
                              name,
                              percent
                            }) => `${name}: ${(percent * 100).toFixed(0)}%`}>
                                  {serviceStats.socialMedia.platformsData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                                </Pie>
                                <Tooltip />
                              </PieChart>
                            </ResponsiveContainer>
                    ...
                    {/* Marketing Services */}
                    {(serviceCategory === 'all' || serviceCategory === 'marketing') && <div className="mb-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              {Object.entries(serviceStats.marketing.services).map(([service, count]) => <div key={service} className="bg-white rounded-lg border p-3">
                                  <p className="text-xs text-gray-500 capitalize">
                                    {service === 'seo' ? 'SEO' : service === 'ppc' ? 'PPC' : service === 'googleAnalytics' ? 'Google Analytics' : 'Google Business Profile'}
                                  </p>
                                  <p className="text-lg font-bold">{count}</p>
                                </div>)}
                            </div>
                          ...
                          <div className="h-60 bg-white rounded-lg border p-4">
                            <ResponsiveContainer width="100%" height="100%">
                              <PieChart>
                                <Pie data={serviceStats.marketing.servicesData} cx="50%" cy="50%" labelLine={false} outerRadius={80} fill="#8884d8" dataKey="value" label={({
                              name,
                              percent
                            }) => `${name}: ${(percent * 100).toFixed(0)}%`}>
                                  {serviceStats.marketing.servicesData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                                </Pie>
                                <Tooltip />
                              </PieChart>
                            </ResponsiveContainer>
                    ...
                    {/* OTT Campaigns */}
                    {(serviceCategory === 'all' || serviceCategory === 'ottCampaigns') && <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              {Object.entries(serviceStats.ottCampaigns.platforms).map(([platform, count]) => <div key={platform} className="bg-white rounded-lg border p-3">
                                  <p className="text-xs text-gray-500 capitalize">{platform}</p>
                                  <p className="text-lg font-bold">{count}</p>
                                </div>)}
                            </div>
                          ...
                          <div className="h-60 bg-white rounded-lg border p-4">
                            <ResponsiveContainer width="100%" height="100%">
                              <PieChart>
                                <Pie data={serviceStats.ottCampaigns.platformsData} cx="50%" cy="50%" labelLine={false} outerRadius={80} fill="#8884d8" dataKey="value" label={({
                              name,
                              percent
                            }) => `${name}: ${(percent * 100).toFixed(0)}%`}>
                                  {serviceStats.ottCampaigns.platformsData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                                </Pie>
                                <Tooltip />
                              </PieChart>
                            </ResponsiveContainer>
                  ...
                  <Card className="lg:col-span-1">
                    ...
                    <CardContent>
                      <div className="h-60">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie data={teamStats.rolesData} cx="50%" cy="50%" labelLine={false} outerRadius={80} fill="#8884d8" dataKey="value" label={({
                            name,
                            percent
                          }) => `${name}: ${(percent * 100).toFixed(0)}%`}>
                              {teamStats.rolesData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                            </Pie>
                            <Tooltip />
                          </PieChart>
                        </ResponsiveContainer>
                  ...
                  <Card className="lg:col-span-2">
                    ...
                    <CardContent>
                      <Table>
                        ...
                        <TableBody>
                          {teamStats.recentActivity.map(activity => <TableRow key={activity.id}>
                        ...
                        <div className="h-60">
                          <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                              ...
                              value: serviceOrderStats.ordersByType.graphics_design
                            }, {
                              name: 'Digital Marketing',
                              value: serviceOrderStats.ordersByType.digital_marketing
                            }, {
                              name: 'Social Media',
                              value: serviceOrderStats.ordersByType.social_media
                            }, {
                              name: 'OTT Campaigns',
                              value: serviceOrderStats.ordersByType.ott_campaigns
                            }]} cx="50%" cy="50%" labelLine={false} outerRadius={80} fill="#8884d8" dataKey="value" label={({
                              name,
                              percent
                            }) => `${name}: ${(percent * 100).toFixed(0)}%`}>
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="h-60">
                          <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                              <Pie data={Object.entries(serviceOrderStats.ordersByStatus).map(([key, value]) => ({
                              ...
                              percent
                                ...
                                {Object.entries(serviceOrderStats.ordersByStatus).map(([key, value]) => <Cell key={`status-${key}`} fill={ORDER_STATUS_COLORS[key as OrderStatus]} />)}
                              </Pie>
                              <Tooltip />
                            </PieChart>
                          </ResponsiveContainer>
                  ...
                  <CardContent>
                    <div className="overflow-x-auto">
                      <Table>
                        ...
                        <TableBody>
                          {serviceOrderStats.teamPerformance.map(member => <TableRow key={member.id}>
