...
const InfluencerListItem = ({
  ...
  return <div onClick={onClick} className={`flex items-center gap-3 py-3 hover:bg-gray-50 rounded-lg px-2 transition-colors cursor-pointer ${isSelected ? 'bg-gray-100' : ''}`}>
  ...
  return <div className="p-4">
      <div className="flex items-start gap-4 mb-6">
        ...
        <div>
          ...
          <div className="mt-4">
            {influencer.niche && <span className="inline-block bg-purple-100 text-purple-800 text-xs px-2 py-1 rounded mr-2">
                {influencer.niche.name}
              </span>}
            {influencer.state && <span className="inline-block bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded mr-2">
                {influencer.state.name}
              </span>}
            {influencer.city && <span className="inline-block bg-green-100 text-green-800 text-xs px-2 py-1 rounded">
      ...
      <div className="border-b">
        <div className="flex">
          <button className={`px-4 py-2 border-b-2 ${activeTab === 'services' ? 'border-primary text-primary' : 'border-transparent'}`} onClick={() => setActiveTab('services')}>
            Services
          </button>
          <button className={`px-4 py-2 border-b-2 ${activeTab === 'prices' ? 'border-primary text-primary' : 'border-transparent'}`} onClick={() => setActiveTab('prices')}>
            Prices
          </button>
          <button className={`px-4 py-2 border-b-2 ${activeTab === 'data' ? 'border-primary text-primary' : 'border-transparent'}`} onClick={() => setActiveTab('data')}>
            Data
          </button>
        </div>
      </div>
      ...
      <div className="mt-4">
        {activeTab === 'services' && <div className="grid grid-cols-2 gap-4">
...
const ContentCard = ({
  ...
  return <div className="rounded-lg overflow-hidden shadow-sm border">
      <div className="h-36 bg-gray-200">
        <img src={`https://picsum.photos/id/${Math.floor(Math.random() * 100)}/300/200`} alt="Content" className="w-full h-full object-cover" />
      </div>
              ...
              <div className="col-span-3 bg-white rounded-lg shadow-sm">
                <div className="p-4 border-b flex items-center justify-between">
                  ...
                  <div className="relative w-1/2">
                    <Input placeholder="Search influencers..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-9" />
                ...
                <div className="max-h-[calc(100vh-200px)] overflow-y-auto">
                  {isInitialLoad ? <InfluencerListSkeleton /> : loading && !isInitialLoad ? <div className="flex items-center justify-center py-8">
                    ...
                    </div> : filteredInfluencers.length === 0 ? <div className="p-8 text-center text-gray-500">
                      <div className="mb-2">
