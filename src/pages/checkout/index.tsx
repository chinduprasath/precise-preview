      ...
      <header className="sticky top-0 z-30 border-b bg-card/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
          ...
          <div className="flex items-center gap-2 rounded-full border bg-card px-3 py-1.5 text-xs font-medium text-muted-foreground">
            <ShieldCheck className="h-4 w-4 text-primary" /> Secure order review
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:py-10">
        <div className="mb-7">
          <div className="mb-2 flex items-center gap-2 text-xs font-medium text-primary">
            <span className="rounded-md bg-primary/10 px-2 py-1">Order #{order.orderNumber}</span>
          </div>
          <h1 className="text-2xl font-bold sm:text-3xl">Review and place your order</h1>
          <p className="mt-1 text-sm text-muted-foreground">Confirm the creator, campaign details, and deliverables before placing your order.</p>
        </div>

        <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1.55fr)_minmax(330px,0.8fr)]">
          <div className="space-y-5">
            <Section title="Campaign & creator profile" icon={Users} badge="Verified creator partner">
            ...
            <Section title="Order details" icon={ReceiptText} badge="Ready to place">
            ...
            <Section title="Campaign deliverables" icon={PackageCheck} badge="Milestone 1 of 1">
          ...
          <aside className="space-y-4 lg:sticky lg:top-6">
            ...
            <Card className="border-border shadow-md">
              <CardHeader className="border-b pb-4"><CardTitle className="text-lg">Order summary</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                ...
                <p className="flex items-center justify-center gap-1.5 text-center text-xs text-muted-foreground"><LockKeyhole className="h-3.5 w-3.5" />Your order details are protected</p>
              </CardContent>
            </Card>
          </aside>
        </div>
...
const Section = ({ title, icon: Icon, badge, children }: { title: string; icon: React.ComponentType<{ className?: string }>; badge?: string; children: React.ReactNode }) => (
  <Card className="border-border shadow-sm"><CardHeader className="border-b pb-4"><div className="flex flex-wrap items-center justify-between gap-3"><CardTitle className="flex items-center gap-2 text-base"><span className="h-5 w-1 rounded-full bg-primary" /><Icon className="h-4 w-4 text-primary" />{title}</CardTitle>{badge && <Badge variant="secondary" className="font-medium text-primary">{badge}</Badge>}</div></CardHeader><CardContent className="pt-5">{children}</CardContent></Card>
);
